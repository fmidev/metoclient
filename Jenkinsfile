    ...
    stages {
        ...
        stage('Build') {
            ...
            steps {
                ...
                sh "ssh fmi@dev.elmo.fmi.fi \"mkdir --parents --mode=755 /fmi/dev/www/test.fmi.fi/metoclient/${env.BRANCH_NAME}/${env.BUILD_NUMBER}\""
                sh "scp -rp dist/ examples/ fmi@dev.elmo.fmi.fi:/fmi/dev/www/test.fmi.fi/metoclient/${env.BRANCH_NAME}/${env.BUILD_NUMBER}/"
                sh "ssh fmi@dev.elmo.fmi.fi \"rm -rf /fmi/dev/www/test.fmi.fi/metoclient/${env.BRANCH_NAME}/latest; cp -rf /fmi/dev/www/test.fmi.fi/metoclient/${env.BRANCH_NAME}/${env.BUILD_NUMBER} /fmi/dev/www/test.fmi.fi/metoclient/${env.BRANCH_NAME}/latest\""
            }
        }
    }
