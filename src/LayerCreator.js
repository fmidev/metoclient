        ...
        ? new TileLayer({
            ...
            previous:
              layer.previous != null
                ? layer.previous
                : [
                    options.layers.find((l) => l['next'] === layer.id),
                  ].map((next) => (next == null ? null : next.id))[0],
            next:
              layer.next != null
                ? layer.next
                : [
                    options.layers.find((l) => l['previous'] === layer.id),
                  ].map((previous) =>
  ...
  static image(layer, options, capabilities) {
    ...
    return new ImageLayer({
      ...
      previous:
        layer.previous != null
          ? layer.previous
          : [options.layers.find((l) => l['next'] === layer.id)].map((next) =>
      ...
      next:
        layer.next != null
          ...
          : [
              options.layers.find((l) => l['previous'] === layer.id),
            ].map((previous) => (previous == null ? null : previous.id))[0],
      legendTitle: layer.legendTitle,
      id: layer.id,
