...
export class MetOClient extends BaseObject {
  ...
  constructor(options = {}) {
    ...
    this.status_ = {};
    this.delay_ = ((options.refreshInterval != null)
      && (options.refreshInterval <= Number.MAX_SAFE_INTEGER) && (options.refreshInterval >= 0))
      ? options.refreshInterval : constants.DEFAULT_DELAY;
    ...
    this.refreshInterval_ = options.refreshInterval
      ? Math.min(Duration.fromISO(options.refreshInterval).valueOf(),
        constants.MAX_REFRESH_INTERVAL) : constants.DEFAULT_REFRESH_INTERVAL;
    this.capabilities_ = {};
    this.legends_ = {};
  ...
  render() {
    return this.updateCapabilities_().then(() => {
      ...
      Object.keys(this.config_.sources).forEach((source) => {
        if ((this.config_.sources[source].times != null)
        && (this.config_.sources[source].times.length > 0)) {
          this.config_.sources[source].tiles = updateSourceTime(this.config_.sources[source].tiles,
            this.config_.sources[source].times.includes(defaultTime)
              ? defaultTime : this.config_.sources[source].times[0]);
        }
      });
      this.updateMap_();
    }).catch((error) => {
  ...
  async updateCapabilities_() {
    ...
    const responses = await Promise.all(this.config_.layers.reduce((urls, layer) => {
      if (layer.source == null) {
        return urls;
      }
      const sourceIds = [layer.source];
      if ((layer.time != null) && (layer.time.source != null)
    ...
    await Promise.all(responses.map((response) => {
      ...
      && (response.requestURL.endsWith(constants.GET_CAPABILITIES_QUERY))) {
        ...
        if (typeof CapabilitiesReader[this.capabilities_[capabKey].type] === 'function') {
          ...
          this.capabilities_[capabKey].data = CapabilitiesReader[type](response.responseText);
        }
      }
      return response;
    }));
  ...
  currentTimeRendered_(event) {
    ...
    if (this.updateNeeded_) {
      ...
      layers.filter((layer) => {
        ...
        const layerConfig = this.config_.layers.find((layerConfig) => layerConfig.id === layerId);
        ...
        if ((prevLayer == null) && (layerConfig != null) && (layerPrevTime != null)) {
          ...
          const index = layers.getArray().findIndex((l) => l.get('metoclient:id') === layer.get('metoclient:id'));
