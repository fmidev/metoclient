...
Internal.prototype.init = function init (map) {
  ...
  this.lineHeight =
    this.getItemsLength() > 0
      ? this.ContextMenu.container.offsetHeight / this.getItemsLength()
      : this.ContextMenu.Html.cloneAndGetLineHeight()
}
...
Internal.prototype.positionContainer = function positionContainer (pixel) {
  ...
  var container = this.ContextMenu.container
...
Internal.prototype.closeMenu = function closeMenu () {
  this.opened = false
  addClass(this.ContextMenu.container, CSS_VARS.hidden)
  this.ContextMenu.dispatchEvent({
...
Html.prototype.createMenu = function createMenu () {
  ...
  if ('items' in this.ContextMenu.options) {
    items = this.ContextMenu.options.defaultItems
      ? this.ContextMenu.options.items.concat(DEFAULT_ITEMS)
      : this.ContextMenu.options.items
  } else if (this.ContextMenu.options.defaultItems) {
    items = DEFAULT_ITEMS
  }
  // no item
...
Html.prototype.addMenuEntry = function addMenuEntry (item) {
  ...
  if (item.items && Array.isArray(item.items)) {
    ...
    sub.style.left =
      this.ContextMenu.Internal.submenu.lastLeft || this.ContextMenu.Internal.submenu.left
    li.appendChild(sub)
...
if (Control) ContextMenu.__proto__ = Control
ContextMenu.prototype = Object.create(Control && Control.prototype)
ContextMenu.prototype.constructor = ContextMenu

/**
 * Remove all elements from the menu.
 */
ContextMenu.prototype.clear = function clear () {
 ...
 * Enable menu
 */
ContextMenu.prototype.enable = function enable () {
  this.disabled = false
}
 ...
 * Disable menu
 */
ContextMenu.prototype.disable = function disable () {
  this.disabled = true
}
 ...
 */
ContextMenu.prototype.getDefaultItems = function getDefaultItems () {
 ...
 */
ContextMenu.prototype.countItems = function countItems () {
 ...
 */
ContextMenu.prototype.extend = function extend (arr) {
  assert(Array.isArray(arr), '@param `arr` should be an Array.')
  arr.forEach(this.push, this)
}

ContextMenu.prototype.isOpen = function isOpen () {
 ...
 */
ContextMenu.prototype.updatePosition = function updatePosition (pixel) {
