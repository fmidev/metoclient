...
export default class TimeSlider {
   ...
   * @memberOf TimeSlider
   */
  constructor (config, container, callbacks, instance, timeConfig) {
    this.instance_ = instance
    this.container_ = container
    this.viewConfig_ = config
    this.callbacks_ = callbacks
    this.timeConfig_ = timeConfig
    this.paper_ = null
  ...
  createPostTools () {
    ...
    stepButton.appendChild(timeStepMenu)

    stepButton.addEventListener('mouseup', (e) => {
      if (stepButton != e.target) return;
  ...
  updateTimeLoaderVis (numIntervalItems) {
    if (!this.viewConfig_['showTimeSlider']) {
      return
    }
